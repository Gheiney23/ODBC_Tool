...
connect_info = ('DRIVER={SQL Server};'
                 'SERVER=server_here;'
...
opname_query =  """SELECT DISTINCT ps.optionname           
                   FROM table_view_1 AS ps (NOLOCK)
                   LEFT JOIN table_view_1 AS p (NOLOCK)
                   ON ps.productid = p.productid
                   WHERE ps.manufacturer = ?
...
specs_query =  """WITH SPECS AS(                                       
                    SELECT
                        ...
                        ps.optionname,
                        ps.optionvalue
                    FROM table_view_1 AS p (NOLOCK)
                    INNER JOIN table_view_2 AS ps (NOLOCK)
